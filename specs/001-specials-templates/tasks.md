...
- [X] T001 Confirm both repos are checked out on `feature/ls-2021-specials` from `develop`: theme
...
- [X] T002 [P] [THEME] Clear the pattern and style transient cache so new/changed pattern and
  style files register: `php -d memory_limit=1024M $(which wp) transient delete --all --network`
  from `wp-content/themes/sd-theme-2026/`
- [X] T003 [P] [THEME] Run Gate 0 baseline per quickstart.md: `phpcs --standard=.phpcs.xml.dist .`,
...
- [X] T004 [P] [THEME] Enumerate every `#special-` anchor referenced in the theme —
...
- [ ] T005 [P] Settle the enquiry-trigger identifier with the line 15/16 owner: whether the
  ...
  **post ID** (`contracts/enquiry-trigger-contract.md`). Record the decision — it changes both
  ...
  `[PLUGIN]` module `modules/enquiry.php`. Must be settled before either is touched. ⟶ **DEFERRED 2026-09-17.** Put to Zared; decision was to defer to the line 15/16 owner. The band ships without the identifier — the button still opens the modal through `#to-modal-enquiry`, per the contract's degraded-behaviour clause. T025/T026/T027 wait on this.
- [X] T006 Verify `lsx/post-connection`'s omission behaviour against real data on dev per
...
- [X] T007 [PLUGIN] Extend `SD\Enhancements\Queries::limit_specials_archive()` in
...
- [X] T008 [PLUGIN] Verify T007 on dev per quickstart Check 1, counted against the data rather
...
- [X] T009 [US1] [THEME] Rewrite `wp-content/themes/sd-theme-2026/templates/archive-special.html`
  ...
  `<main>`; research R-01)
- [X] T010 [US1] [THEME] Create `wp-content/themes/sd-theme-2026/patterns/template-archive-special.php`
  ...
  docblock
- [X] T011 [US1] [THEME] In `patterns/template-archive-special.php`, add a `core/query` block
...
- [X] T012 [US1] [THEME] In `patterns/template-archive-special.php`, build the offer band inside
...
- [X] T013 [US1] [THEME] In the band from T012, add centred `core/post-title` for the offer name
...
- [X] T014 [US1] [THEME] In the band from T012, add the enquire `core/button` reading "Enquire
  ...
  here
- [X] T015 [US1] [THEME] In `patterns/template-archive-special.php`, add `core/query-pagination`
...
- [X] T016 [P] [US1] [THEME] Extend `wp-content/themes/sd-theme-2026/styles/sections/cards/special-card.json`
...
- [X] T017 [US1] [THEME] Below the query in `patterns/template-archive-special.php`, confirm which
...
- [X] T018 [US1] [THEME] Confirm in `patterns/template-archive-special.php` that the band reflows
...
- [ ] T019 [US1] Verify quickstart Check 2 on dev for **every** migrated offer, not a sample:
  ...
  class names in T012 do not match `special-card.json`) (SC-003) ⟶ **PARTIAL.** Verified by execution on **local** against six seeded fixtures covering every validity shape, plus connections; not yet on dev, because the plugin half cannot be deployed there from here (MCP is content-only, and nothing is committed). Re-run on dev after both branches deploy.
...
- [X] T020 [US2] [THEME] In the meta-row placeholder from T013, add `core/paragraph` bound to
...
- [X] T021 [US2] [THEME] In the same meta row, add `core/paragraph` bound to
  `lsx/post-connection` → `destination_to_special`, labelled `Destinations:` the same way
  (FR-014)
- [X] T022 [US2] [THEME] In the same meta row, add `core/post-terms {"term":"travel-style"}`
  labelled `Travel Style:`, linking to that taxonomy term's archive (FR-014, FR-015)
- [X] T023 [US2] Verify quickstart Check 3 on dev across the full migrated set: labels present
...
- [ ] T024 [US2] Verify quickstart Check 4 on dev: activate each meta link on an offer with
  ...
  pass), and the travel-style link opens that term's archive (FR-015) ⟶ **PARTIAL.** Connection links render as `#to-modal-{id}` overlay triggers, confirmed on local. Whether the overlay itself opens is line 16 and is deferred verification, as this task allows.
...
- [ ] T025 [US3] [THEME] Using the identifier decided in T005, add the per-band attribute to the
  ...
  (`contracts/enquiry-trigger-contract.md`) ⟶ **BLOCKED on T005.**
- [ ] T026 [US3] [PLUGIN] Only if T005's identifier needs server-side reading: extend
  ...
  a client-side-only identifier. ⟶ **BLOCKED on T005.** Likely skipped entirely if the identifier turns out to be client-side only.
- [ ] T027 [US3] Verify quickstart Check 5 on dev, not a spot check: on a page showing four
  ...
  with a visible focus indicator (SC-007, FR-018, FR-029) ⟶ **BLOCKED on T005.** SC-007 is verified when the line 15/16 surface lands, not waived.
...
- [X] T028 [US4] [PLUGIN] Create `wp-content/plugins/sd-enhancements-2026/modules/specials.php`
...
- [X] T029 [US4] [PLUGIN] Leave `lsx_to_settings['special_disable_single']` off. Do **not** enable
  ...
  R-07)
- [X] T030 [P] [US4] [THEME] Delete `wp-content/themes/sd-theme-2026/templates/single-special.html`
  (FR-024)
- [X] T031 [US4] [PLUGIN] Add a render-time filter in
...
- [ ] T032 [US4] Verify quickstart Check 6 on dev: follow every anchor from the T004 list plus the
  ...
  (SC-008) ⟶ **PARTIAL.** The T004 test set is a single anchor, `#special-five-rivers-special`. Anchor injection itself is verified on local: four bands, four correct `id="special-{slug}"`. ⚠️ That offer (dev 58260) is a **draft**, so it is not on the archive at all and the anchor cannot resolve until it is published — a content decision for the 458-unpublished-items question, not a code fault.
- [ ] T033 [US4] Verify quickstart Check 7 on dev: for every offer slug
  ...
  in the theme returns nothing (SC-005) ⟶ **PARTIAL.** Verified on local for a published and a draft offer: single-hop 301 to the archive, which returns 200. The theme grep returns nothing ✓. Per-slug sweep over dev's 42 offers waits on deployment.
...
- [X] T034 [US5] [PLUGIN] Add a scheduled sweep to
...
- [X] T035 [US5] [PLUGIN] In the sweep from T034: make retiring an already-`draft` offer a no-op
...
- [X] T036 [US5] [PLUGIN] Confirm no hook is added to `tour-operator` and `Post_Expiration` is not
...
- [ ] T037 [US5] Verify quickstart Check 8a on dev — test the write, not the schedule: close one
  ...
  expired in it. **Restore dev's data afterwards** (SC-006a) ⟶ **PARTIAL.** Every mechanic verified on local against real fixtures — retire, meta preserved, idempotent second run, republish restores the offer intact with no stale retirement, offers with no end date never touched. Dev run waits on deployment. Dev data untouched.
- [ ] T038 [US5] Verify quickstart Check 8 on dev: temporarily close every published offer's
  ...
  empty list under working controls (FR-013, spec edge cases) ⟶ **PARTIAL.** Empty-state verified on local by deleting the fixtures: banner, title and introduction still render with the `query-no-results` message and no broken pagination.
...
- [X] T039 [P] [THEME] Re-run Gate 0 — `phpcs --standard=.phpcs.xml.dist .`,
  `find patterns templates parts -name '*.php' -exec php -l {} \;` — clean, against the finished
  feature (quickstart Gate 0)
- [X] T040 [P] [THEME] Run quickstart Check 12: `grep -rnE '#[0-9a-fA-F]{3,8}|rgb\(|font-family:\s*["A-Za-z]' patterns/template-archive-special.php styles/sections/cards/special-card.json`
...
- [ ] T041 [P] Run quickstart Check 9 on dev: automated accessibility scan with no critical or
  ...
  (SC-009, SC-010, FR-030) ⟶ **PARTIAL.** Exactly one `<main>` ✓ (pattern 1, template 0). Automated accessibility scan and contrast-over-image at every width are Zared's pass per AGENTS.md rule 5 — see the hand-over note.
- [X] T042 [P] [THEME] Run quickstart Check 10: open the Specials archive template in the Site
...
- [ ] T043 Run quickstart Check 11: side-by-side comparison of live `/specials/` against the
  ...
  fix-verify loop (SC-012) ⟶ **Zared's review.** Not run here by design (AGENTS.md rule 5).
- [X] T044 Verify SC-006 at hand-over: fetch `special-sitemap.xml` and confirm zero individual
  offer URLs. This is search-plugin configuration owned by the deployment/redirects line — record
  the result, do not implement a fix here (FR-023) ⟶ **DONE, and it fails today.** `special-sitemap.xml` on live returns 200 and advertises all five individual offer URLs alongside the archive. Recorded, not fixed — deployment/redirects line owns it (FR-023).
- [X] T045 [P] [THEME] Update `wp-content/themes/sd-theme-2026/CHANGELOG.md` per Keep a Changelog
  1.1.0, tagged LS-2021
- [X] T046 [P] [PLUGIN] Update `wp-content/plugins/sd-enhancements-2026/CHANGELOG.md` per Keep a
  Changelog 1.1.0, tagged LS-2021
- [X] T047 Locate what produces the live `301` on southerndestinations.com today (outside
