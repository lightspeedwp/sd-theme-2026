...
| `typography.fontFamilies` | **6 presets, 10 bundled WOFF2 faces** (`heading`, `belleza`, `body`, `accent`, `la-belle-aurore`, `monospace`). Open Sans ×7, Belleza, La Belle Aurore and Joe Hand are bundled. **No Optima face** — the client licence covers desktop use only (LS-2641), so `heading` keeps its `Optima, Belleza, sans-serif` stack and resolves to Belleza where Optima is not installed locally. Each typeface needs its *own* preset: WordPress overwrites a `fontFace`'s `fontFamily` with the first name of its preset's stack. → `style.md` §3.4, §3.7 · `assets/fonts/LICENCES.md` |
