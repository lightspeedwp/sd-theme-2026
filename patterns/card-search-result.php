 ...
 *
 * The plate is **`brand-600`**, not `brand-500` — Zared's call, 2026-09-17. It
 * is the same ground the rail's keyword-search button takes, so the two brand
 * tiles on the page read as one colour rather than two near-misses. It also
 * carries the label: `base` on `brand-600` (#BC5B18) is **4.5:1**, where
 * `brand-500` (#CC7F16) was 3.2:1 — under AA for a `100`-sized label.
 *
...
<div class="wp-block-columns is-style-listing-card-list has-neutral-200-background-color has-background">
	...
	<div class="wp-block-column is-vertically-aligned-stretch" style="padding-top:var(--wp--preset--spacing--30);padding-right:var(--wp--preset--spacing--30);padding-bottom:var(--wp--preset--spacing--30);padding-left:var(--wp--preset--spacing--30)">
		...
		<div class="wp-block-columns">
			 ...
			 *
			 * ⚠️ `fontSize` has to be written on **every** paragraph in the
			 * panel, not just on the panel column — and even then it does not
			 * reach the excerpt. theme.json styles `core/paragraph`, whose
			 * block selector is `p`, so global styles emit
			 * `:root :where(p){font-size:var(--wp--preset--font-size--300)}`.
			 * At (0,1,0) that beats *inheritance* from the column's
			 * `.has-200-font-size`, so any bare `<p>` in the panel renders at
			 * 300 unless it carries the class itself. That is why the three
			 * bound paragraphs below each set `fontSize: 200` of their own
			 * although the column already does. The `core/post-terms` blocks
			 * beside them need no such care — they render a `div`, not a `p`.
			 *
			 * The excerpt cannot be answered that way: `core/post-excerpt` puts
			 * the size class on its wrapper `div` and the text sits in an inner
			 * `<p class="wp-block-post-excerpt__excerpt">` the attribute never
			 * reaches. `assets/styles/core-post-excerpt.css` answers it once,
			 * for every excerpt in the theme, with `font-size: inherit` on that
			 * inner paragraph.
			 */
			?>
			...
			<div class="wp-block-column is-vertically-aligned-stretch has-neutral-100-background-color has-background has-200-font-size" style="padding-top:var(--wp--preset--spacing--20);padding-right:var(--wp--preset--spacing--20);padding-bottom:var(--wp--preset--spacing--20);padding-left:var(--wp--preset--spacing--20);line-height:var(--wp--custom--line-height--body);flex-basis:35%">
				...
				<!-- wp:paragraph {"metadata":{"name":"Price Rating","bindings":{"content":{"source":"sd/post-meta","args":{"key":"price_rating","format":"price-band"}}}},"className":"lsx-price-rating-wrapper","prefix":"Price Rating:","prefixBold":true,"fontSize":"200","blockVisibility":{"controlSets":[{"id":1,"enable":true,"controls":{"location":{"ruleSets":[{"enable":true,"rules":[{"field":"postType","operator":"any","value":["accommodation"]}]}]}}}]}} -->
				<p class="lsx-price-rating-wrapper has-200-font-size"></p>
				<!-- /wp:paragraph -->
				...
				<!-- wp:paragraph {"metadata":{"name":"Location — Accommodation","bindings":{"content":{"source":"lsx/post-connection","args":{"key":"destination_to_accommodation"}}}},"className":"lsx-destination-to-accommodation-wrapper","prefix":"Location:","prefixBold":true,"fontSize":"200","blockVisibility":{"controlSets":[{"id":1,"enable":true,"controls":{"location":{"ruleSets":[{"enable":true,"rules":[{"field":"postType","operator":"any","value":["accommodation"]}]}]}}}]}} -->
				<p class="lsx-destination-to-accommodation-wrapper has-200-font-size"></p>
				<!-- /wp:paragraph -->

				<!-- wp:paragraph {"metadata":{"name":"Location — Tour","bindings":{"content":{"source":"lsx/post-connection","args":{"key":"destination_to_tour"}}}},"className":"lsx-destination-to-tour-wrapper","prefix":"Location:","prefixBold":true,"fontSize":"200","blockVisibility":{"controlSets":[{"id":1,"enable":true,"controls":{"location":{"ruleSets":[{"enable":true,"rules":[{"field":"postType","operator":"any","value":["tour"]}]}]}}}]}} -->
