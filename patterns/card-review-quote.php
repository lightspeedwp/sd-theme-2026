 ...
 *
 * ⚠️ **The wrapper markup has to match The Icon Block's `save()` exactly**, or
 * the editor flags the block as containing unexpected content and offers to
 * recover it. It was authored here with `has-icon-color` and no `transform`,
 * which is neither what 2.0.0 saves nor what an earlier version saved, so the
 * card opened broken from the day it was written. Repaired in the Site Editor on
 * dev (wp_template 65942) on 2026-09-04 and imported here.
 *
 * Icon Block 2.0.0's `save()` (build/index.js) writes, for these attributes:
 *
 * - `has-icon-color` **only when `iconColorValue` is set** — a custom colour.
 *   With a palette `iconColor` it is absent, and `has-<slug>-color` carries the
 *   colour instead.
 * - `transform: rotate(0deg) scaleX(1) scaleY(1)` **always**, from the block's
 *   rotate and flip controls, even at their defaults.
 *
 * So: no `has-icon-color`, and the transform is not optional.
 *
