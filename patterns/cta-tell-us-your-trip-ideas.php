<?php
/**
 * Title: CTA — Tell Us Your Trip Ideas
 * Slug: sd-theme-2026/cta-tell-us-your-trip-ideas
 * Description: The enquiry band live runs beneath every Tour Operator single — a script heading over the two office numbers side by side, with the "Send us an Email" action beneath, on the warm-grey ground.
 * Categories: sd-theme-2026/call-to-action, sd-theme-2026/tour-operator
 * Keywords: cta, enquiry, contact, phone, call, email, safari gurus, trip ideas
 * Viewport Width: 1200
...
/*
 * The default heading of live's `sd_call_info_section()`.
 *
 * `partials/footer-cta.php` switches the band's title on body class, and this
 * is the "everything else" branch — the one every Tour Operator single gets,
 * measured on /tour/botswana-victoria-falls-safari/ 2026-08-28 where it sits
 * inside `.lsx-full-width-base-small` between the review carousel and the
 * related-tours shelf.
 *
 ...
 * body-class conditional becomes in a block theme: which pattern each template
 * includes.
 *
 * The two are separate files rather than one pattern with an editable heading
 * because a pattern is a copy, not a link — a shared file whose heading is
 * overwritten on insertion would give the person building a page no way to tell
 * which variant they had. Two named entries in the inserter do.
 *
 * ⚠️ **They must stay in step.** Any change to the offices, the icon or the
 * action belongs in both. The only line that may differ is the heading.
 */
?>
<!-- wp:group {"tagName":"section","metadata":{"name":"CTA - Tell us your trip ideas"},"align":"full","className":"is-style-tinted-page-section","style":{"spacing":{"blockGap":"var:preset|spacing|40"}},"layout":{"type":"constrained"}} -->
<section class="wp-block-group alignfull is-style-tinted-page-section">

	<!-- wp:heading {"textAlign":"center","className":"is-style-script-accent","style":{"elements":{"link":{"color":{"text":"var:preset|color|brand-500"}}}},"textColor":"brand-500","anchor":"h-tell-us-your-trip-ideas"} -->
	<h2 class="wp-block-heading has-text-align-center is-style-script-accent has-brand-500-color has-text-color has-link-color" id="h-tell-us-your-trip-ideas"><?php esc_html_e( 'Tell us your trip ideas and we\'ll send you ours!', 'sd-theme-2026' ); ?></h2>
	<!-- /wp:heading -->
